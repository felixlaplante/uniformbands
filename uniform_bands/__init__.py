...
def _student(F_lo, F_hi, alpha, eps):
    ...
    return {
        "lower": np.clip(mean_lo + q_lo * std_lo, 0, 1),
        "upper": np.clip(mean_hi + q_hi * std_hi, 0, 1),
    }


def get_bands(F_lo, F_hi, alpha=0.05, eps=1e-8, method="uniform"):
    """
    ...
    return _student(F_lo, F_hi, alpha, eps)
